...
export class Client {
     ...
     * @return Error
     */
    updateguide(body: UpdateGuideModel | undefined): Promise<boolean> {
    ...
    protected processUpdateguide(response: Response): Promise<boolean> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 400) {
            return response.text().then((_responseText) => {
     ...
     * @return Error
     */
    all(pageNumber: number | undefined): Promise<ActivityModelPagedList> {
    ...
    protected processAll(response: Response): Promise<ActivityModelPagedList> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 400) {
            return response.text().then((_responseText) => {
            ...
            let resultdefault: any = null;
            let resultDatadefault = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            resultdefault = ActivityModelPagedList.fromJS(resultDatadefault);
            return resultdefault;
            });
        }
    }
...
export class AccountClient {
    ...
    delete(returnUrl: string | null | undefined): Promise<void> {
...
export class ActivityModelPagedList implements IActivityModelPagedList {
    pageNumber?: number;
    readonly itemsCount?: number;
    readonly items?: ActivityModel[] | undefined;

    constructor(data?: IActivityModelPagedList) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
        ...
        if (_data) {
            ...
            if (Array.isArray(_data["items"])) {
                ...
                for (let item of _data["items"])
                    (<any>this).items!.push(ActivityModel.fromJS(item));
            }
        }
    }

    static fromJS(data: any): ActivityModelPagedList {
        data = typeof data === 'object' ? data : {};
        let result = new ActivityModelPagedList();
        result.init(data);
        return result;
    }
...
export interface IActivityModelPagedList {
    pageNumber?: number;
    itemsCount?: number;
    items?: ActivityModel[] | undefined;
}
