...
1. Install the Firebase SDK in the app directory:

   ```bash
   npm install firebase
   ```

2. Open `src/services/pushNotificationService.ts` in the app and uncomment the FCM implementation block.

3. The service worker (`sw.js`) generated by `vite-plugin-pwa` already includes the Workbox runtime caching. For background message handling, create a `firebase-messaging-sw.js` in the `public/` folder:
